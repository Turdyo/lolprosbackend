...
def getPlayerDb(player):
    try:
        ...
        response = {"response": "Le joueur n'existe pas"}
        return response
...
def getTeamDb(team):
    try:
        ...
        response = {"response": "La Team n'existe pas"}
        return response
...
def leaderboard(request):
    ...
    for account in accounts:
        player = {
            ...
            "rank": account.rank,
            "LP": account.leaguePoints,
            "team": account.player.team.name.capitalize()
            if account.player.team
            else None,
...
def getPlayerDiscord(request, discordID):
    ...
    try:
        ...
        return JsonResponse(
            {"response": "Aucun joueur associé a ce discordID"}, status=404
        )
